...
from evaluate.model_eval import evaluate_on_CoopNavi
import wandb
import psutil
        ...
        for t in range(argus.train.per_episode_length):
            ...
            if done or (t >= argus.train.per_episode_length - 1):
                ...
                if (ep+1)%argus.evaluate.eval_every_n_ep == 0:
                    eval_episode_rewards, eval_agents_rewards = evaluate_on_CoopNavi(agent=model, env=env, argus=argus, render=argus.evaluate.render)
    ...
    for ep in range(argus.train.max_episodes):
        ...
        for t in range(argus.train.per_episode_length):
            ...
            if done or (t >= argus.train.per_episode_length - 1):
                ...
                replay_buffer._store_exprience()
                if (ep+1) % argus.evaluate.eval_every_n_ep == 0:
                    eval_episode_rewards, eval_agents_rewards = evaluate_on_CoopNavi(agent=madre, env=env, argus=argus, render=argus.evaluate.render)
                ...
                if (ep + 1) % argus.control.update_every_n_ep == 0:
                    ...
                    ep_add_update_time_consum += time.time() - star_interact
