...
from evaluate.model_eval import evaluate_on_reference
import wandb
...
def prepare_for_train(model, replay_buffer, train_recoder, env, argus):
    for ep in range(argus.train.before_train_episode):
        ...
        for t in range(argus.train.per_episode_length):
            ...
            replay_buffer.traj_buffer._store_action_data(data=copy.deepcopy(np.reshape(action_index, [1, argus.env.agent_num, argus.env.n_action])))
            replay_buffer.traj_buffer._store_action_prob_data(data=copy.deepcopy(np.reshape(action_prob, [1, argus.env.agent_num, argus.env.n_action])))
            ...
            if done or (t >= argus.train.per_episode_length - 1):
                ...
                if (ep+1)%argus.evaluate.eval_every_n_ep == 0:
                    eval_episode_rewards, eval_agents_rewards = evaluate_on_reference(agent=model, env=env, argus=argus, render=argus.evaluate.render)
...
def main():
    ...
    algorithm, argus = get_variant_config(scenario="reference")
    ...
    for ep in range(argus.train.max_episodes):
        ...
        for t in range(argus.train.per_episode_length):
            ...
            if done or (t >= argus.train.per_episode_length - 1):
                ...
                replay_buffer._store_exprience()
                if (ep+1)%argus.evaluate.eval_every_n_ep == 0:
                    eval_episode_rewards, eval_agents_rewards = evaluate_on_reference(agent=madre, env=env, argus=argus, render=argus.evaluate.render)
                    if eval_episode_rewards > argus.control.render_threshold:
                        argus.evaluate.render = True
                    else:
                ...
                if (ep + 1) % argus.control.update_every_n_ep == 0:
                    ...
                    for _ in range(argus.control.update_repeat_n_time):
                        critic_loss, actor_loss, reward_loss, relative_ratio_var = train_model(
                            agent=madre, argus=argus, replay_buffer=replay_buffer, ep=ep, time_step=train_recoder.time_step)
