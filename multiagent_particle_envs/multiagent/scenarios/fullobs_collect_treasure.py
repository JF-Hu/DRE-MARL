...
from multiagent_particle_envs.multiagent.core_trea import World, Agent, Landmark
...
class Scenario(BaseScenario):
    ...
    def deposit_reward(self, agent, world):
        ...
        if shape:  # reward can optionally be shaped
            ...
            if len(dists_to_holding) > 0:
                ...
                rew -= 1.0 * np.linalg.norm(closest_avg_dist_vect)
    ...
    def collector_reward(self, agent, world):
        ...
        if agent.holding is None and shape:
            rew -= 1.0 * min(world.cached_dist_mag[t.i, agent.i] for t in
                             self.treasures(world))
        elif shape:
            rew -= 1.0 * min(world.cached_dist_mag[d.i, agent.i] for d in
    ...
    def observation_(self, agent, world):
        n_visible = 20  # number of other agents and treasures visible to each agent
        # get positions of all entities in this agent's reference frame
    ...
    def observation(self, agent, world):
        return self.general_observation(agent, world)

    def general_observation(self, agent, world):

        self_color = agent.color
        ...
        for other in world.agents:
            ...
            if other.collector:
                ...
                other_pos.append(other.color)

        # get positions of all entities in this agent's reference frame
        treasure_pos = []
        for entity in world.landmarks:
            treasure_pos.append(entity.state.p_pos - agent.state.p_pos)
        # entity colors
        treasure_color = []
        for entity in world.landmarks:
            treasure_color.append(entity.color)

        return np.concatenate([agent.state.p_vel] + [agent.state.p_pos]+[self_color]+comm+other_pos+treasure_pos+treasure_color)