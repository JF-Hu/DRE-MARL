...
class Scenario(BaseScenario):
    ...
    def is_touch(self, agent):
        delta_pos = agent.state.p_pos - agent.goal_a.state.p_pos
        dist = np.sqrt(np.sum(np.square(delta_pos)))
        dist_min = agent.size + agent.goal_a.size
    ...
    def adversary_reward(self, agent, world):
        ...
        pos_rew = min(min(agent_dist), 2.0)
        neg_rew = 5.0*np.sqrt(np.sum(np.square(agent.goal_a.state.p_pos - agent.state.p_pos)))
